...
image = np.fromfile('Nema/nema.emission.sinogram', dtype='<f').reshape(293, 280) / np.fromfile('Nema/nema.ctac.sinogram', dtype='<f').reshape(293, 280)
...
for i in range(1, 180):
    for filt in ['ramp', 'hamming', 'hanning']:
        for interpolation in ['nearest', 'linear', 'cubic']:
            ...
            plt.imshow(reconstruction_fbp, cmap=plt.cm.Greys_r)
            plt.savefig('results/fbp/Nema/'+ str(i) + '_' + str(interpolation) + '_' + str(filt) + '.png')
            plt.show()
